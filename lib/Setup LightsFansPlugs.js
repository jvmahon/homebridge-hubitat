...
{
	return "light";
/* if 	(accessory.capabilities.includes("FanControl") {return "fan"}
	else if 	(accessory.capabilities.includes("Switch")
				&& (accessory.capabilities.includes("Level")
				) return "light"
	else if 	(accessory.capabilities.includes("FanControl") {return "fan"}
	else return "none";
	*/
}
exports.setupLightsFansPlugs = function (that, services)
{
	...
	let Service 		= that.api.hap.Service;
		...
		if (that.currentAccessory.capabilities.includes("Switch"))
		{
			...
			switch(accessoryType(that.currentAccessory) )
			{
				case "switch":
				{
				...
				case "light": // A simple non-dimming lightbulb
				{
					var thisService = new Service.Lightbulb()
					var SwitchControl	= thisService.getCharacteristic(Characteristic.On);
					break;
