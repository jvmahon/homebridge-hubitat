...
class HubitatSystem 
{
	constructor(log, config, api)
	{
		...
		this.UpdateDeviceTraits = [];// This is an array of HomeKit Services and Characteristics that want to receive event notifications. Each entry includes a sub-array .notifyObjects which is an array of HomeKit Services and Characteristics placed that get placed into this array by the updateOnHubEvents() function. The index to this.UpdateDeviceTraits[] is the ID of the Hubitat device (the first parameter to updateOnHubEvents() ).
...
// The following "send" is specifically to control Home Safety Monitor.
	...
	async refreshAll() // Grabs device data from Hubitat just to be sure Homebridge remains in sync. There is no reason it should ever get out of sync, but this is an extra precaution.
	{
		...
		// Loop through all the device IDs of devices that want update. Remember, UpdateDeviceTraits is indexed by the Hubitat Device ID #
		for (deviceID in that.UpdateDeviceTraits)
		{
			...
			// For a given Hubitat device ID, there may be multiple Services / Characteristics that could be updated. Loop thorugh them.
			for(var thisHomekitObject of that.UpdateDeviceTraits[deviceID].notifyObjects)
			{
				...
				// x is the indext to the reportTypes arrray
				for(var thisAttributeName of thisHomekitObject.reportTypes)
				{
					...
					if ((fullDeviceData.attributes[thisReportName]) === undefined) continue
				...
				for(var x in thisHomekitObject.reportTypes)
				{
					...
					if (!(fullDeviceData.attributes[thisHomekitObject.reportTypes[x]])) continue
	...
	// reportTypeArray is a listing of Hubitat attributes.
		HomeKitObject.id = id;
