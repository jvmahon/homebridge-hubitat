	...
	listenForChanges()
	{
		...
		that.webSocket
			...
			.on('connection', function connection(ws)
				{
					that.log(chalk.green(`Established connection to Hubitat WebSocket EventStream`));
					ws.isAlive = true;
				})
			...
			.on('close', function close()
				{
					that.log(chalk.red(`Lost connection to Hubitat WebSocket EventStream`));
					clearInterval(interval);
				});
			
		const interval = setInterval(function ping() 
			{
				// if (that.webSocket.isAlive === false) return that.webSocket.terminate();
