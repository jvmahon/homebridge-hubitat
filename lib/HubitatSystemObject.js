...
class HubitatSystem 
{
	...
	async refreshAll()
	{
		...
		for (deviceID in that.Devices)
		{
			var fullDeviceData	= this.allDevices.find( function(currentElement) { return (currentElement.id == deviceID)})

			for(var thisHomekitObject of that.Devices[deviceID].notifyObjects)
			{
				report.value = (fullDeviceData.attributes[thisHomekitObject.reportType]);
				
				that.log(`For id: ${JSON.stringify(deviceID)}, report type:  ${thisHomekitObject.reportType}, new value is:  ${report.value}`);
	...
	//
		...
		console.log(red(`Registered object with id = ${id} to receive updates of type ${HomeKitObject.reportType}. Array is now length ${this.Devices[id].notifyObjects.length}`));
		}
		else
		{
