...
class HubitatSystem 
{
	...
	send = function(id, ...args)
	{
		...
		fetch(control)
			.then( function(response) {
				that.log(`Sent to device id: ${id} the command ${args.join("/")}.`);
			})
		...
		that.webSocket.on('message', function (data)
		{
			...
			if (receivedData.source == "DEVICE")
			{
				...
				for(var thisObject of that.Devices[receivedData.deviceId].notifyObjects)
				{
					...
					{
						that.log(chalk.yellow(`Emitting a data value ${chalk.cyan(receivedData.value)} for device id: ${chalk.cyan(receivedData.deviceId)} and report type ${chalk.cyan(thisObject.reportType)}.`));
