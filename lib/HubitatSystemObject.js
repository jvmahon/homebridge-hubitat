		...
		function sendfunction(cb)
		{
			...
			fetch(control)
				.then( function(response) {
					...
					that.log(`Queued to device id: ${cyan(id)} labeled ${cyan(fullDeviceData.label)} the command ${cyan(args.join("/"))}.`);
			...
			.on('message', function (data)
				{
					var receivedData = JSON.parse(data);
								that.log(chalk.green(`Received from device id: ${chalk.cyan(receivedData.deviceId)} a data value ${chalk.cyan(receivedData.value)} in report type ${chalk.cyan(receivedData.name)} with Display Name ${chalk.cyan(receivedData.displayName)}.`));
					...
					if (receivedData.source == "DEVICE")
					{
						...
						for(var thisObject of that.Devices[receivedData.deviceId].notifyObjects)
						{
							...
							{
								that.log(chalk.yellow(`Received from device id: ${chalk.cyan(receivedData.deviceId)} a data value ${chalk.cyan(receivedData.value)} in report type ${chalk.cyan(receivedData.name)} with Display Name ${chalk.cyan(receivedData.displayName)}.`));
