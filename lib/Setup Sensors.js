...
exports.setupSensor = function (that, services)
{
		...
		&& ( that.currentAccessory.attributes.battery !== null))		// Some device with a battery may also be USB charged, in which case, the battery attribute is set to null, so don't add the battery device!.
	{
		...
		var ChargingState = thisSensorService.getCharacteristic(Characteristic.ChargingState).setInitialValue(2);
...
*/
	...
	if (that.currentAccessory.capabilities.includes("TemperatureMeasurement"))
	{
		...
		let initialTemp = (that.platformConfig.temperatureUnits == "F")
