...
exports.HSMRefresh = function(that)
{
	...
	if (that.config.createHSMSecuritySystemDevices == "Intrusion") {
		that.log(`Calling Refresh on Home Safety Monitor to update HomeKit with Current HSM status.`);
		...
		fetch(control).then( response => response.json())
			.then( data => {
				...
				that.UpdateDeviceTraits[0]?.notifyObjects
					.filter( (it) => {return it.hubitatAttributesOfInterest.includes("hsmStatus")})
					.forEach( (thisHomekitObject) => {
		...
		case "Water":
		{
			...
			thisSecuritySystem			
				...
				.on('HubValueChanged', (HubReport, HomeKitObject)=>  { 
						...
						} else if ((HubReport.name == "hsmStatus") && (HubReport.value == "allDisarmed")) {
								HomeKitObject.getCharacteristic(Characteristic.SecuritySystemCurrentState).updateValue(3); 
								// HomeKitObject.getCharacteristic(Characteristic.SecuritySystemTargetState).updateValue(3); 									
						}
					}
