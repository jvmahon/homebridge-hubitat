...
exports.setupHomeSafetyMonitor = function (that, services)
{
	...
	if (that.platformConfig.createHSMSecritySystemdevices.includes("Intrusion"))
	{

		var HSMIntrusionSecuritySystem = new Service.SecuritySystem("Intrusion", 0)
		var HSMWaterSecuritySystem = new Service.SecuritySystem("Water", 1)
		
		HSMWaterSecuritySystem.getCharacteristic(Characteristic.SecuritySystemCurrentState).setProps({validValues:[0, 3, 4]});
		HSMWaterSecuritySystem.getCharacteristic(Characteristic.SecuritySystemTargetState).setProps({validValues:[0, 3]});
		
		var HSMSmokeSecuritySystem = new Service.SecuritySystem("Smoke", 2)
		
		var currentState = HSMIntrusionSecuritySystem.getCharacteristic(Characteristic.CurrentState)
		var targetState = HSMIntrusionSecuritySystem.getCharacteristic(Characteristic.TargetState)	
		
		HSMIntrusionSecuritySystem			
		...
		services.push(HSMIntrusionSecuritySystem);	
		services.push(HSMWaterSecuritySystem)
		services.push(HSMSmokeSecuritySystem)	
	}	
}