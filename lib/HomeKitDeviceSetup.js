...
console.log(cyan("that is: " + Object.getOwnPropertyNames(that.api)));
	...
	console.log(chalk.cyan(`currentAccessory is: ${Object.getOwnPropertyNames(that.currentAccessory)}`));
	console.log(chalk.cyan(`currentAccessory is: ${JSON.stringify(that.currentAccessory)}`));
	...
	switch (that.currentAccessory.type) 
	{
		...
		case "Generic Zigbee Bulb":
		{
			...
			Switch.id = that.currentAccessory.id;
				...
				.on('set', function(newHomekitValue, callback, context)
							{
								...
								switch(newHomekitValue == true)
								{
									...
									case (true) : 
									{ 
										that.HubData.send(Switch.id, "switch", "on"); 
										break;
									}
									case (false): 
									{
										that.HubData.send(Switch.id, 'switch', "off"); 
										break;
									}
								}
			...
			var Level = thisService.addCharacteristic(new Characteristic.Brightness())
				Level.id = that.currentAccessory.id;
	...
	console.log(chalk.yellow(`Setting up Sensors with that object: ${Object.getOwnPropertyNames(that.platformConfig)}`));
